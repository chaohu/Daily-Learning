...
int get_msb(int x) {
    /* Shift by w-8 */
    unsigned shift_val = (sizeof(int)-1u)<<3;
    /* Arithmetic shift */
 ...
 * For example 0xFF00 -> 0x0100, and 0x6600 --> 0x200
 */
unsigned test_rightmost_one(unsigned x) {
    int mask;
...
int any_MSB_one(unsigned x) {
    ...
    unsigned msb_mask = 0xFFu << (sizeof(int)-1u << 3);
    return !!(x & msb_mask);
}
...
int odd_ones(unsigned x) {
    ...
    /* Use !! to convert from unsigned to int */
    return !!(p1 & 1u);
}
/* $end odd_ones_solve */
...
/* Assume 1 <= n <= w */
int test_fits_bits(int x, unsigned n)
{
    int tmax_n = (1<<(n-1)) -1;
    int tmin_n = -tmax_n - 1;
...
/* Do rotating right shift.  Assume 0 <= n < w */
unsigned test_rotate_right(unsigned x, unsigned n)
{
    unsigned result = x;
    int i;
    for (i = 0; i < n; i++) {
...
/* Do rotating left shift.  Assume 0 <= n < w */
unsigned test_rotate_left(unsigned x, unsigned n)
{
...
int mul3div4(int x) {
    int mul3 = x + (x<<1);
    int mul3_mask = mul3 >> ((sizeof(int)<<3)-1u);
...
int mul5div8(int x) {
    int mul5 = x + (x<<2);
    int mul5_mask = mul5 >> ((sizeof(int)<<3)-1u);
