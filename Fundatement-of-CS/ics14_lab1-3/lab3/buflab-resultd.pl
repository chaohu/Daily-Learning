#!/usr/bin/perl
#######################################################################
# buflab-resultd.pl - The CS:APP buffer bomb result server
#
# The result server is a simple, special-purpose HTTP server that
# collects real-time results from bombs that the students are defusing.  
#
...
# Main routine
##############
my $server_port = $Buflab::RESULTD_PORT;
my $client_port;
my $client_iaddr;
my $client_dname;
my $server_dname;
...
while (1) {
    ...
    #
    if ((length($request_hdr) > $Buflab::MAXHDRLEN) or 
    ...
    #
    unless (open(LOGFILE, ">>$Buflab::LOGFILE")) {
		log_die("Error: Unable to open $Buflab::LOGFILE for appending: $!");
    }
