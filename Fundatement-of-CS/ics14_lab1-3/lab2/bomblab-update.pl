...
#
# Copyright (c) 2004-2-11, R. Bryant and D. O'Hallaron, All rights reserved.
#
#######################################################################
...
foreach $bombid (sort {$RESULTS{$b}[2] <=> $RESULTS{$a}[2] ||
    ...
    unless (-e $bombdir and -d $bombdir) {
    ...
    print WEB "<td align=center>$bombcount</td>\n"; 
...
sub read_logfile {
    ...
    while ($line = <LOGFILE>) {
        ...
        ($hostname, $time, $userid, $labid, $result) = 
            split(/\|/, $line, 5);
        if (!$hostname or !$time or !$userid or !$labid or !$result) {
