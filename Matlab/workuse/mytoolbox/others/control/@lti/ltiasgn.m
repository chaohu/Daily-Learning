...
%   $Revision: 1.2 $  $Date: 1998/08/26 21:48:35 $
...
if isempty(lrhs),
   ...
   if all(iscolon(3:end))
      ...
      L.ioDelayMatrix(indices{:}) = [];
   else
      % Model deletion: update Td if multi-dimensional
      if ndims(L.ioDelayMatrix)>2,
         L.ioDelayMatrix(indices{:}) = [];
      end
...
if zerorhs,
   lrhs.ioDelayMatrix = 0;
   lrhs.InputDelay = 0;    % Used below as marker for zero RHS
   lrhs.OutputDelay = 0;
end
if isequal(size(lrhs.ioDelayMatrix),[1 1]),
...
%%%%%%%%%%%%%%%%%%%%%%%%
if ~any(L.ioDelayMatrix(:)) & ~any(lrhs.ioDelayMatrix(:)),
   % All zero delays
   L.ioDelayMatrix = zeros(newsizes(1:2));
else
   % Expand L.ioDelayMatrix when non empty (i.e., LHS is not created by the assignment)
   if length(sizes)>ndims(L.ioDelayMatrix) & ~isequal(L.ioDelayMatrix,[]),
      L.ioDelayMatrix = repmat(L.ioDelayMatrix,[1 1 sizes(3:end)]);
   end
   % Expand LRHS.TD when not scalar
   srd = size(lrhs.ioDelayMatrix);
   if length(rsizes)>length(srd),
      lrhs.ioDelayMatrix = repmat(lrhs.ioDelayMatrix,[1 1 rsizes(3:end)]);
   end
   % Perform assignment
   L.ioDelayMatrix(indices{:}) = lrhs.ioDelayMatrix;
end
...
elseif isequal(sizes,[0 0]),
   ...
   if strcmp(indices{1},':'),
      ...
      % Absorb delays that are no longer pure input delays in ioDelayMatrix
      if any(notid),
         Dmid = zeros(newsizes);
...
% Update I/O ioDelayMatrix for models which lost their input or output delays
if any(Dmid(:)) | any(Dmod(:)),
   % Revisit: remove next IF when ND add is OK
   if ndims(L.ioDelayMatrix)<length(newsizes),
      L.ioDelayMatrix = repmat(L.ioDelayMatrix,[1 1 newsizes(3:end)]);
   end
   L.ioDelayMatrix = L.ioDelayMatrix + Dmid + Dmod;
end

% In the array case, keep only one copy if ioDelayMatrix is the same for all models
L.ioDelayMatrix = tdcheck(L.ioDelayMatrix);
...
if ischar(indices{1}) | all(ismember(1:length(L.OutputName),indrow)),
   ...
   if clash, 
      warning('Input name clash. Conflicting RHS names ignored.')
   end
   ...
   if clash,
      warning('Input group clash. RHS groups ignored.')
   elseif isempty(Glhs) & ~isempty(Grhs),
      L.InputGroup = groupasgn(L.InputGroup,indcol,Grhs);
   end
end

% OutputName and OutputGroup
if ischar(indices{2}) | all(ismember(1:length(L.InputName),indcol)),
   ...
   if clash, 
       warning('Output name clash. Conflicting RHS names ignored.')
   end
   ...
   if clash,
      warning('Output group clash. RHS groups ignored.')
   elseif isempty(Glhs) & ~isempty(Grhs),
