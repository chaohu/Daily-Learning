...
%   $Revision: 1.10 $  $Date: 1998/10/01 20:12:31 $
...
if sys.Ts==0,
   ...
   for k=1:prod(sizes(3:end)),
      ...
      % Direct feedthrough makes covariance infinite
      dwd = dk*w*dk';
      ifeed = find(abs(dwd) > eps * norm(dwd,1));   % nonzero entries of D*W*D'
      if ~isempty(ifeed),
         warn2 = 'Continuous systems with direct feedthrough have infinite output covariance.';
         pk(ifeed) = Inf;
      end
