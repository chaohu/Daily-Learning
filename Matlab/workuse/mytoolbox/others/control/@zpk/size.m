...
%   $Revision: 1.13 $  $Date: 1998/10/01 20:12:30 $
...
if ni==1,
   if no==0,
      ...
      nu = sizes(2);
      if nd==2,
         disp(sprintf('Zero-pole-gain models with %d output%s and %d input%s.',...
            ny,s(1,ny>1),nu,s(1,nu>1)));
      else
         ArrayDims = sprintf('%dx',sizes(3:end));
         disp(sprintf('%s array of zero-pole-gain models',ArrayDims(1:end-1)))
