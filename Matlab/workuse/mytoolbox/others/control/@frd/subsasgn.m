...
%   $Revision: 1.9 $  $Date: 1998/09/18 20:45:58 $
...
elseif isa(rhs,'double'),  % RHS is a ND array
   ...
   if isa(freqIndices,'numeric') & ~all(diff(round(freqIndices))>0)
...
if isa(freqIndices,'numeric')           % bound the size of logicals
   if islogical(freqIndices)
      if length(freqIndices)<freqSize | ...
            length(freqIndices)>freqSize
         error('Logical frequency index has wrong length.');
      end
      freqIndices = find(freqIndices);
   else
      % Take into account that MATLAB indexing will round doubles
      %      freqIndices = round(freqIndices);
      if ~isempty(freqIndices) & ...
            (min(freqIndices)<0.5 | ...
            max(freqIndices)>=freqSize+0.5)
