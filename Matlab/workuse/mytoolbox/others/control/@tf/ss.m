...
%      $Revision: 1.13 $  $Date: 1998/10/01 20:12:42 $

% Effect on other properties: keep all LTI parent fields

sys = varargin{1};

% Handle syntax ss(a,b,c,d,sys) with sys of class TF
if ~isa(sys,'tf'), 
  nlti = 0;
...
ni = nargin;
if ni>2,
   error('Conversion from TF to SS: too many input arguments.');
elseif ~isproper(sys),
   error('Improper system. Conversion to state space is not possible');
elseif ni==2 & ~isstr(varargin{2}),
   error('Conversion from TF to SS: second argument must be a string');
end
...
% Normalize first denominator entry
for m=1:prod(sizes)
   d1 = den{m}(1);
   den{m} = den{m}/d1;
   num{m} = num{m}/d1;
...
for m=1:prod(sizes(3:end)),
   if co(m)<=ro(m),
      ...
      for j=1:Nu,
         [aj,bj,cj,dj] = sioreal(num(:,j,m),den(:,j,m));
      ...
      for i=1:Ny,
         [ai,bi,ci,di] = sioreal(num(i,:,m),den(i,:,m));
...
SSsys = ss(a,b,c,d,mindelay(sys.lti,'alldelay'),'nx',Nx);
if Ny>1 | Nu>1,
   % Balance result when not already done by COMPREAL
   SSsys = ssbal(SSsys);
end
