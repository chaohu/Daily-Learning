...
%       $Revision: 1.11 $  $Date: 1998/07/16 20:04:08 $
...
% Handle syntax zpk(z,p,k,sys) with sys of class TF
if ~isa(sys,'tf'),   
  nlti = 0;
...
% Error checking
if nargin>1,
  error('Conversion from TF to ZPK: too many input arguments.');
end
...
if any(sizes==0),
   ...
   for i=1:prod(sizes),
      ...
      [knum,z{i}] = getkr(sys.num{i});
      % Poles are the roots of denominator:
      [kden,p{i}] = getkr(sys.den{i});
      % Set gain
      k(i) = knum/kden;
   end % for i

   ch = sys.Variable;
   if strcmp(ch,'s') | strcmp(ch,'z'),
      ZPKsys = zpk(z,p,k,sys.lti);
   else
      ZPKsys = zpk(z,p,k,sys.lti,'Variable',ch);
   end
end
