...
%   $Revision: 1.17 $  $Date: 1998/09/18 17:58:03 $
   ...
   for k=1:prod(sizes(3:end)),
      ...
      while j<=nu,
         ...
         if ~any(nij),
            ...
            [sys.num{i,j,k},sys.den{i,j,k}] = ndmin(nij,dij);
         end
   ...
   for k=1:max(nsys1,nsys2),
      ...
      for i=1:prod(sizes2(1:2)),
         ...
         if ~any(nk{i}),
            ...
            [nk{i},dk{i}] = ndmin(nk{i},conv(scalden,dk{i}));
         end
      end
      sys.num(:,:,k2) = nk;
      sys.den(:,:,k2) = dk;
   ...
   for k=1:max(nsys1,nsys2),
      ...
      for i=1:prod(sizes1(1:2)),
         ...
         if ~any(nk{i}),
            ...
            [nk{i},dk{i}] = ndmin(nk{i},conv(dk{i},scalden));
         end
      end
      sys.num(:,:,k1) = nk;
      sys.den(:,:,k1) = dk;
...
function [num,den] = ndmin(num,den)
%NDMIN  Eliminates common leading zeros created by operations 
%       like  s * 1/s
