function [Xlims,Ylims] = axeslims(NicRespObj,RespObj);
%AXESLIMS sets the axes limits for Nichols Response Objects

%   Copyright (c) 1986-98 by The MathWorks, Inc.
%   $Revision: 1.1 $  $Date: 1998/05/21 19:38:04 $
...
if strcmp(AllRespProps.XlimMode,'auto')
   ...
   if ~isempty(Xdata),
      ...
      Xmax = max(Xdata(:));
      TestLims = Lims;
      if (Lims(2)-Xmax) <= 0.05*(TestLims(2)-TestLims(1)),
         Lims(2) = TestLims(2) + 0.02*(TestLims(2)-TestLims(1));
      end
      if (Xmin-Lims(1)) <= 0.05*(TestLims(2)-TestLims(1)),
         Lims(1) = TestLims(1) - 0.02*(TestLims(2)-TestLims(1));
      end
