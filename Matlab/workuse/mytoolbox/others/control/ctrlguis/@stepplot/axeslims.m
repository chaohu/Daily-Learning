function [Xlims,Ylims] = axeslims(StepRespObj,RespObj);
%AXESLIMS sets the axes limits for Step Response Objects
...
if strcmp(AllRespProps.XlimMode,'auto')
   ...
   if strcmpi(get(AllRespProps.Parent,'Tag'),'ResponseGUI') & ~isempty(onSys),
      Xmax = LocalGetSettlingTime(StepRespObj,Xmax,onSys);
      Lims(2)=Xmax;
   end
...
PRLines = [findall(DispAx,'Tag','PeakResponseMarker','Marker','none');
   findall(DispAx,'Tag','RiseTimeMarker','Marker','none')];
...
dcLines = findall(DispAx,'Tag','SteadyStateMarker','Marker','none');
if ~isempty(dcLines),
   Parent = get(dcLines,{'Parent'});
   Xlims = get(cat(1,Parent{:}),{'Xlim'});
   set(dcLines,{'Xdata'},Xlims);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Xmax = LocalGetSettlingTime(StepRespObj,Xmax,onSys);

ST = get(StepRespObj,'SettlingTimeValue');
OnTime=ST(onSys);
