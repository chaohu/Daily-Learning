...
%CALCOPT calculates Plot Options for Initial Condition Response Plots
...
for ctInd = 1:length(Ind);
   ...
   for ctrow = 1:size(AllHandles,1),
      ...
      for ctModel = 1:prod(size(Handles)),
         ...
         case 'peakresponsevalue',
            %----Remove the first point, in case the max. response *is* the initial condition
            absY=absY(2:end);
            [garb,indMax]=max(absY);
            Ymax=Y(indMax);
            Xmax=X(indMax+1);
            Yvals(ctrow,1,ctModel) = Ymax;
            Xvals(ctrow,1,ctModel)= Xmax;
