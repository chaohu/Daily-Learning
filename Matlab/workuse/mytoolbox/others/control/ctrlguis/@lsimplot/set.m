function Out = set(LsimRespObj,varargin)
...
ni = nargin;
no = nargout;
if ~isa(LsimRespObj,'response'),
   % Call built-in SET. 
   builtin('set',LsimRespObj,varargin{:});
   return
elseif no & ni>2,
   error('Output argument allowed only in SET(LsimRespObj) or SET(LsimRespObj,Property)');
end

% Get properties and their admissible values when needed
if ni>1,  flag = 'lower';  else flag = 'true';  end
if ni<=2,
   [AllProps,AsgnValues] = pnames(LsimRespObj,flag);
else
   AllProps = pnames(LsimRespObj,flag);
end

% Handle read-only cases
if ni==1,
   % SET(StepRespObj) or S = SET(LsimRespObj)
   if no,
...
elseif ni==2,
   % SET(LsimRespObj,'Property') or STR = SET(LsimRespObj,'Property')
   Property = varargin{1};
   if ~isstr(Property),
...
% Now left with SET(LsimRespObj,'Prop1',Value1, ...)
name = inputname(1);
if rem(ni-1,2)~=0,
   error('Property/value pairs must come in even number.')
end

ContextMenu = get(LsimRespObj,'UicontextMenu');
RespObj = LsimRespObj.response;
RespObjProp = [];
for i=1:2:ni-1,
...
if ~isempty(RespObjProp)
   ...
   LsimRespObj.response = RespObj;
end

% Make sure Response and UIcontextMenu have latest Simulation Response Object
set(ContextMenu.Main,'UserData',LsimRespObj);

% Finally, assign sys in caller's workspace
if ~isempty(name),
   assignin('caller',name,LsimRespObj)
end
